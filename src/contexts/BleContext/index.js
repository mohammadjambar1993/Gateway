...
export const BleProvider = (props) => {
  ...
  const [settingsPromptModalVisible, setSettingsPromptModalVisible] = useState(false)
  ...
  const [encryptionModal, setEncryptionModal] = useState(ENCRYPTION_MODAL_INIT_STATE)
      ...
      if (null !== lastJsonMessage) {
        ...
        if ('ble:snapshot' === code && noError) {
          if (data.state) {
            ...
            if (typeof data.state.suspended !== 'undefined'
        ...
        if ('ble:encryption:set' === code) {
          if (error) {
            handleEncryptionModalError(typeof error === 'string' ? error : 'An unknown error occured.' )
          } else {
  ...
  const promptUserForSettings = (peripheralId) => {
    if (settingsPromptPromise.current && settingsPromptPromise.current.resolve && settingsPromptPromise.current.reject) {
    ...
    if (pod && pod.settings && pod.settings.csvFileName) {
      const settings = {
        ...
        saveNumber: ( pod.settings.saveNumber === false ) ? '0' : `${pod.settings.saveNumber}`,
  ...
  const startPod = async (peripheralId) => {
    try {
      ...
      const settings = {
        ...
        saveNumber: ( _settings.saveNumber === '0' ) ? false : parseInt(_settings.saveNumber),
  ...
  const handleSaveSettings = () => {
    ...
    for (let i = 0; i < ks.length; i++) {
      ...
      if (validator instanceof RegExp) {
        console.log('match regex', { reg: settingsFormValues[ks[i]].match(settingsValidation[ks[i]])})
        isInvalid = (settingsFormValues[ks[i]].match(settingsValidation[ks[i]]) === null)
        ...
        console.log('unsupported validator, will pass data to backend', validator)
      }
      if (isInvalid) {
  ...
  */
  ...
  const openEncryptionModal = ({ peripheralId, key, digit }) => {
    console.log('++ openEncryptionModal', {peripheralId, connected: connectedPods[peripheralId], dc: podList[peripheralId], podList, connectedPods })
  ...
  return (
    ...
    >
      ...
      {appState === AppStatus.RUNNING ?
        props.children : <Container style={{marginTop: '150px'}}>
        <Card className="text-center">
          <Card.Header>App Status</Card.Header>
          <Card.Body>
            <Card.Title>App {appSuspended.is ? 'is suspended' : appStatus}</Card.Title>
            <Card.Text>
        ...
        <Container fluid>
          ...
          <Navbar.Collapse className="justify-content-end">
            ...
            <OverlayTrigger
              ...
              trigger={['click']}
              rootClose
              overlay={<Popover id="popover-appinfo" className="popover-appinfo">
                <Popover.Header as="h3">App info</Popover.Header>
                <Popover.Body>
                  { appSuspended.is ? <Alert variant={'warning'}>
                  ...
                  {debugInfo.enabled && (<>
                    ...
                    <ListGroup horizontal={'md'} className="my-2">
                      <ListGroup.Item>Total: {prettyBytes(debugInfo.data.diskspace.size)}</ListGroup.Item>
                      <ListGroup.Item>Free: {prettyBytes(debugInfo.data.diskspace.free)}</ListGroup.Item>
                      <ListGroup.Item>Path: {debugInfo.data.diskspace.diskPath}</ListGroup.Item>
                    </ListGroup>

                    <h6>Memory Usage</h6>
                    <ListGroup className="my-2">
                      <ListGroup.Item>
                        Heap: {debugInfo.data.heapTotal}/{debugInfo.data.heapUsed} (total/used)
                      </ListGroup.Item>
                      <ListGroup.Item>
                        External: {debugInfo.data.external} <br />
                        <small>refers to the memory usage of C++ objects bound to JavaScript objects managed by V8.</small>
                      </ListGroup.Item>
                      <ListGroup.Item>
                        RSS: {debugInfo.data.rss} <br /> 
                        <small>Resident Set Size, is the amount of space occupied in the main memory device (that is a subset of the total allocated memory) for the process, including all C++ and JavaScript objects and code.</small>
                      </ListGroup.Item>
                    </ListGroup>
                  </>)}
            ...
            >
              <Navbar.Text style={{cursor: 'pointer'}} as="div" className="me-4">
                <FontAwesomeIcon className={`me-1 text-${appStatusBackground}`} icon={faCircle} /> App {appStatus}
              </Navbar.Text>
            </OverlayTrigger>)}

            <OverlayTrigger overlay={<Tooltip id="tooltip-socket">Socket Url {socketUrl}</Tooltip>}>
              <Navbar.Text style={{cursor: 'pointer'}} className="me-4">
                <FontAwesomeIcon className={`me-1 text-${connectionStatusBackground}`} icon={faCircle} /> Socket is {connectionStatus}
              </Navbar.Text>
            </OverlayTrigger>
      ...
      <Modal show={settingsPromptModalVisible} onHide={handleSettingsPromptModalClose}>
        ...
        <Modal.Body>
          ...
          <Form>
            <Form.Group className="mb-3" controlId="formSettingsPartId">
              ...
              <Form.Control onChange={(e) => settingsFormValueUpdate('partId', e.target.value)} value={settingsFormValues.partId} type="text" placeholder="Enter your Participantion ID." />
            </Form.Group>

            <Form.Group className="mb-3" controlId="formSettingsTaskName">
              <Form.Label>Task Name</Form.Label>
              <Form.Control onChange={(e) => settingsFormValueUpdate('taskName', e.target.value)} value={settingsFormValues.taskName} type="text" placeholder="Enter task name" />
            </Form.Group>

            <Form.Group className="mb-3" controlId="formSettingsFileName">
              <Form.Label>CSV Filename</Form.Label>
              <Form.Control onChange={(e) => settingsFormValueUpdate('csvFileName', e.target.value)} value={settingsFormValues.csvFileName} type="text" placeholder="Enter CSV Filename" />
              <Form.Text muted>This value will be prepended to files generated by this session.</Form.Text>
            </Form.Group>

            <Form.Group className="mb-3" controlId="formSettingsSaveInterval">
              <Form.Label>Save Interval</Form.Label>
              <Form.Control onChange={(e) => settingsFormValueUpdate('saveInterval', e.target.value)} value={settingsFormValues.saveInterval} type="text" placeholder="Enter save interval" />
              <Form.Text muted>Interval in seconds to dump data into files </Form.Text>
            </Form.Group>

            <Form.Group className="mb-3" controlId="formSettingsSaveDuration">
              <Form.Label>Save Number</Form.Label>
              <Form.Control onChange={(e) => settingsFormValueUpdate('saveNumber', e.target.value)} value={settingsFormValues.saveNumber} type="text" placeholder="Enter save duration" />
              <Form.Text muted>Number of files to save, total save duration will be calculated base on this, (0) will run indefinitely</Form.Text>
            </Form.Group>
            ...
            <Form.Group className="mb-3" controlId="formSettingsTestRun">
              <Form.Check 
                ...
                onChange={(e) => settingsFormValueUpdate('test', !settingsFormValues.test)}
              />
              <Form.Text muted>Test mode will behave the same as regular except for saving files.</Form.Text>
            </Form.Group>
      ...
      <GraphModal
        ...
        signalOptions={selectedGraphPod ? connectedPods[selectedGraphPod].metadata.signalOptions : []}
        data={graphData}
        firmwareType={selectedGraphPod ? connectedPods[selectedGraphPod].metadata.firmwareType : ''}
        ECGSampleCount={selectedGraphPod ? connectedPods[selectedGraphPod].metadata.ECGSampleCount : 24}
        ECGPacketInterval={selectedGraphPod ? connectedPods[selectedGraphPod].metadata.ECGPacketInterval : 75}
      />

      <EncryptionModal
        show={encryptionModal.visible}
        podIdentifier={encryptionModal.peripheralId !== null && (<>{podList[encryptionModal.peripheralId].advertisement.localName} ({podList[encryptionModal.peripheralId].metadata.serialNumber})</>)}
